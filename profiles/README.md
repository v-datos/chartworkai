...
| [`investigation`](investigation.md) | evidence-backed findings | not required | every claim → archived source at an evidence tier |
| [`deployed-service`](deployed-service.md) | a deployed service + infra | not required | config + image digest + job URI → traceable release |
